...
## Phase 2: Staff Directory & DM Logic
- [x] Task: Implement Staff List in Chat Sidebar (d852270)
    - [x] Fetch users from `users` collection.
    - [x] Update `<ChatPage />` to display staff.
- [ ] Task: Implement Private Channel ID Logic
    - [ ] Create utility to generate consistent room IDs for pairs of users.
- [ ] Task: Update `<ChatWindow />` for DMs
    - [ ] Handle `recipientId` and `isPrivate` flag during message creation.
- [ ] Task: Write tests for DM room generation and filtering.
- [ ] Task: Conductor - User Manual Verification 'Phase 2: Staff Directory & DM Logic' (Protocol in workflow.md)
